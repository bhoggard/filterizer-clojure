(ns user
  ...
  (:require
    ...
    [tristanmedia.filterizer.core :refer [start-app]]))
...
(defn dev-prep!
  ...
  (integrant.repl/set-prep! (fn []
                              (-> (tristanmedia.filterizer.config/system-config {:profile :dev})
                                  (ig/expand)))))

(defn test-prep!
  []
  (integrant.repl/set-prep! (fn []
                              (-> (tristanmedia.filterizer.config/system-config {:profile :test})
                                  (ig/expand)))))
